...
自定义 OpenAI 兼容服务使用 `CUSTOM_OPENAI_API_KEY`。不要为这些变量添加 `VITE_` 前缀，否则 Vite
可能把它们暴露到浏览器构建中。后端会读取项目根目录的 `.env.local`，并按“临时 Key → 已记住本地 Key →
项目默认环境变量”的优先级、按服务商独立解析。SQLite 只保存 Endpoint、模型、发现结果和脱敏验证状态；前端
只接收 Key 来源和配置状态，绝不回显 Key。
